...
<write_scout_survey>**Always complete a manifest-scoped Scout deviation check (ephemeral report, no on-disk artifact) before writing implementation code**, BECAUSE writing without surveying risks re-implementing functionality already present in manifest modules, or violating naming and error-handling conventions established nearby. The Scout reads only files listed in the ScoutManifest (targets + context arrays) — not an unbounded codebase sweep. For each module in the manifest, the Scout produces a structured per-module report confirming or flagging each `<spec:depth>` claim: (1) entry points — actual count vs. declared (≤3), flagged if different; (2) seam category — actual vs. declared (`in-process`, `file`, `http`, etc.), flagged if different; (3) hidden complexity — does the implementation match the declared boundary, or is it absent, leaky, or wider than claimed? Deviations are informational only — they do not block the Composer. The depth section was written at plan time; code evolves. Flag divergences so the Summarizer can note them; write code against the actual state, not the stale claim.</write_scout_survey>
...
<judge_reinvention>**Always flag as a blocker any diff that re-implements functionality the manifest-scoped Scout deviation report would have surfaced**, BECAUSE the Scout reads manifest files (targets + context from the ScoutManifest) — not a full codebase sweep — and any reinvention it would have caught is a signal that the Composer either skipped the Scout step or read beyond its scope and still missed an existing implementation. Reinvention duplicates behaviour the existing module already hides, undermining locality — bugs and future changes must now be made in two places instead of one.</judge_reinvention>
